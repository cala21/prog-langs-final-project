object Lab4 extends jsy.util.JsyApplication {
  ...
  def strictlyOrdered(t: Tree): Boolean = {
    val (b, _) = t.foldLeft((true, None: Option[Int])){
      (acc, d) => (acc, d) match {
        ...
        case ((true, Some(x)), y) => if (x < y) (true, Some(y)) else (false, Some(y))
      } 
    }
    b
  }
  ...
  def typeInfer(env: Map[String,Typ], e: Expr): Typ = {
    ...
    e match {
      ...
      case Obj(fields) => {
        val tfields = fields.foldLeft(Map(): Map[String,Typ]) {
          (acc, field) => 
            val f = field._1
            val t = typ(field._2)
            acc + (f -> t)
...
//        //Anonymous function
//        case None => params.foldLeft(true){(b,param) => if (param._1 == x) false else b} match {
//          case true => Function(p, params, tann, subst(e1))
//          case _ => e
//        } 
//        //Potential recursive function
//        case Some(f) => (f == x) match {
//          case true => e
//          case _ => params.foldLeft(true){(b,param) => if (param._1 == x) false else b} match {
//            case true => Function(p, params, tann, subst(e1))
//            case _ => e
//          }
//        }
//      }
      ...
      case Obj(fields) => {
        val nfields =fields.foldLeft(Map(): Map[String,Expr]) {
         (newFields, field) => newFields + (field._1 -> subst(field._2))
        };
        Obj(nfields)
      }
